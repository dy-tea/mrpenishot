...
fn generate_request_method(iface Interface, request Message, opcode int) string {
	...
	if has_new_id {
		out.write_string(new_id_arg_name)
	}
	out.writeln(' {')
	...
	if has_new_id {
		if new_id_arg.interface_name != '' {
			...
			out.write_string(if request.is_destructor {
				'wl.wl_marshal_flag_destroy, '
			} else {
				'0, '
			})
        ...
        proxy: proxy
			...
			out.write_string(if call_args.len > 0 {
